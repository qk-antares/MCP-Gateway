...
3. 当添加新的MCP Server时，网关自动调用该服务器的list_tools接口获取工具列表，并经过一个Embedding模型将工具的name+description转化为向量存储在本地数据库中，以便后续的语义搜索。这个本地数据库尽可能轻量化，必须快速实现一个原型，后续再考虑优化。

### MCP Server Gateway API

MCP Server Gateway只提供特定的元工具，比如：
1. discover_tools(intent)：根据输入的意图进行语义搜索，返回匹配的Tools列表（mcp_server+tool_name+desciption，不包含参数schema）。语义搜索可能需要依赖一个Embedding模型，根据intent和tool_name+desciption语义向量余弦相似度返回top-k Tools
2. list_tools()：列出所有MCP Servers下所有Tools的name+description（节省占大头的inputSchema的token），这样不依赖小模型
3. get_tool_schema(server, tool_name)：获取指定工具的完整调用参数schema
4. invoke_tool(server, tool_name, args)：调用指定工具并返回结果
